 ...
 * @returns {Array<import("eslint").Linter.Config>} An array of ESLint configurations for TypeScript with strict rules
 */
export default function loadConfig(config: IConfigOptions): Array<Linter.Config> {
	const typescriptFiles: Array<string> = ["**/*.ts", "**/*.tsx"];

	const typedPresetConfigs: Array<Linter.Config> = formatConfig([...tseslint.configs.recommendedTypeChecked, ...tseslint.configs.strictTypeChecked, ...tseslint.configs.stylisticTypeChecked] as unknown as Array<Linter.Config>).map((presetConfig: Linter.Config): Linter.Config => {
		const parserOptions: Record<string, unknown> = presetConfig.languageOptions?.parserOptions && typeof presetConfig.languageOptions.parserOptions === "object" ? (presetConfig.languageOptions.parserOptions as Record<string, unknown>) : {};
	...
	return [
		...typedPresetConfigs,
		{
			...
			plugins: {
				[formatPluginName("typescript")]: tseslint.plugin as unknown as NonNullable<Linter.Config["plugins"]>[string],
			},
			rules: {
